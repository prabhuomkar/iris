...
class Things(Component):
  ...
  def __init__(self, db, oid, filename, mediaitem_url):
    ...
    self.MODEL_WEIGHTS = 'resnet152.pth'
    self.MODEL_RESULT_FILE =  'imagenet_classes.txt'
    self.MODEL_RESULT_MAPPING = {
  ...
  def get_inference_results(self):
    ...
    print(f'got top categories: {result_classes}')
  ...
  def upsert_entity(self, data):
    ...
    for cat_class in data:
      ...
      result = self.db['entities'].find_one_and_update(
        ...
        {'$addToSet': {'mediaItems': ObjectId(self.oid)}, '$set': {'name': cat_class, 'previewMediaItem': ObjectId(self.oid)}},
        upsert=True,
  ...
  def process(self):
    try:
      ...
      ic_result = self.get_inference_results()
      if 'content_categories' not in ic_result:
        ic_result['content_categories'] = []
      if 'classes' not in ic_result:
        ic_result['classes'] = []
      content_categories = list(set(ic_result['content_categories']))
      classes = list(set(ic_result['classes']))
