...
class Things():
  ...
  def __init__(self, db):
    ...
    self.model = torch.jit.load('checkpoints/things-model.pt')

  def _get_inference_input(self, filename):
    """Returns image tensor from the input file"""
    input_tensor = self.input_preprocess(Image.open(filename))
